...
@section('title', __('adjustment.Edit_Adjustment'))
@section('content-header', __('adjustment.Edit_Adjustment'))
            ...
            <div class="card-body">
                <form action="{{ route('adjustments.update', $adjustment) }}" method="POST" id="adjustmentForm">
                    ...
                    <div class="form-section">
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="product_id" class="form-label required-field">{{ __('adjustment.Product') }}</label>
                                <select name="product_id" id="product_id" class="form-control @error('product_id') is-invalid @enderror" required>
                                    <option value="">{{ __('adjustment.Select_Product') }}</option>
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" 
                                            {{ old('product_id', $adjustment->product_id) == $product->id ? 'selected' : '' }} 
                                ...
                                <div class="stock-info" id="stockInfo">
                                    <strong>{{ __('adjustment.Current_Stock') }}:</strong> 
                            ...
                            <div class="form-group">
                                ...
                                <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                    ...
                                    <option value="increase" {{ old('type', $adjustment->type) == 'increase' ? 'selected' : '' }}>
                                        {{ __('adjustment.Increase') }}
                                    </option>
                                    <option value="decrease" {{ old('type', $adjustment->type) == 'decrease' ? 'selected' : '' }}>
                    ...
                    <div class="form-section">
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="quantity" class="form-label required-field">{{ __('adjustment.Quantity') }}</label>
                            ...
                            <div class="form-group">
                                <label for="adjusted_at" class="form-label required-field">{{ __('adjustment.Adjusted_At') }}</label>
                                <input type="datetime-local" name="adjusted_at" id="adjusted_at" 
                                    class="form-control @error('adjusted_at') is-invalid @enderror" 
                                    value="{{ old('adjusted_at', $adjustment->adjusted_at->format('Y-m-d\TH:i')) }}" required>
                                @error('adjusted_at')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="form-section">
                        <div class="form-group">
                            ...
                            <textarea name="reason" id="reason" class="form-control @error('reason') is-invalid @enderror" 
                                rows="4" placeholder="{{ __('adjustment.Enter_reason') }}" maxlength="500">{{ old('reason', $adjustment->reason) }}</textarea>
                            @error('reason')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="character-count" id="reasonCount">
                                <span id="reasonCurrent">{{ strlen(old('reason', $adjustment->reason)) }}</span>/500
                            </div>
                        </div>
                    </div>

                    <div class="form-section">
                        <h3 class="form-section-title">{{ __('adjustment.Additional_Information') }}</h3>
                        <div class="additional-info">
                            <div class="info-item">
                                <span class="info-label">{{ __('adjustment.Adjusted_By') }}:</span>
                                <span class="info-value">{{ $adjustment->user->getFullname() }}</span>
                            </div>
                            <div class="info-item">
                                <span class="info-label">{{ __('common.Created_At') }}:</span>
                                <span class="info-value">{{ $adjustment->created_at->format('d M Y, H:i') }}</span>
                            </div>
                            <div class="info-item">
                                <span class="info-label">{{ __('common.Updated_At') }}:</span>
                                <span class="info-value">{{ $adjustment->updated_at->format('d M Y, H:i') }}</span>
                            </div>
                        </div>
                    </div>
