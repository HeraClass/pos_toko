            ...
            <div class="">
                <form action="{{route('orders.index')}}">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            <button class="btn btn-outline-primary" type="submit"><i class="nav-icon fas fa-filter"></i>
                                {{ __('order.filter') }}</button>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-secondary btnShowInvoice" data-toggle="modal"
                                    ...
                                    data-customer-name="{{ $order->getCustomerName() }}" data-total="{{ $order->total() }}"
                                    data-received="{{ $order->receivedAmount() }}" data-items="{{ json_encode($order->items) }}"
                                ...
                                @if($order->total() > $order->receivedAmount())
                                    ...
                                    <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#partialPaymentModal"
                                    ...
                                    <div class="modal fade" id="partialPaymentModal" tabindex="-1" role="dialog"
                                        aria-labelledby="partialPaymentModalLabel" aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form id="partialPaymentForm" method="POST"
                                                        action="{{ route('orders.partial-payment') }}">
                                                        @csrf
                                                        <input type="hidden" name="order_id" id="modalOrderId" value="">
                                                        <div class="form-group">
                                                            <label for="partialAmount">Enter Amount to Pay</label>
                                                            <input type="number" class="form-control" step="0.01" id="partialAmount"
                                                                name="amount"
                                                                value="{{ $order->total() - $order->receivedAmount() }}">
                                                        </div>
...
@section('model')
    ...
    <div class="modal fade" id="modalInvoice" tabindex="-1" role="dialog" aria-labelledby="modalInvoiceLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalInvoiceLabel">Toko Kelontong Pak Dedi POS</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <script>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <span class="float-right"> <strong>Status:</strong> ${receivedAmount == 0 ?
                    '<span class="badge badge-danger">{{ __('order.Not_Paid') }}</span>' :
